...
public class BoardTestSuite {
    ...
    void testAddTaskListFindUsersTasks() {
        ...
        List<Task> tasks = project.getTaskLists().stream()
                ...
                .filter(t -> t.getAssignerUser().equals(user))
                .collect(toList());
        //Then
        assertEquals(2, tasks.size());
        assertEquals(user, tasks.get(0).getAssignerUser());
        assertEquals(user, tasks.get(1).getAssignerUser());
    }
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        long tasksNum = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .count();

        long daysValue = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
